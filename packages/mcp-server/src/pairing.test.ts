...
function manager(now?: () => number) {
	return createPairingManager(TOKEN, { now, delayMs: 0 });
}

describe("createPairingManager", () => {
	it("mints 8-char codes drawn from the unambiguous alphabet", () => {
		const { code, ttlMinutes } = manager().mint();
	...
	it("mints a different code each time", () => {
		...
		expect(pairing.mint().code).not.toBe(pairing.mint().code);
	});
	...
	it("invalidates the old code when a new one is minted", async () => {
		const pairing = manager();
		const first = pairing.mint().code;
		const second = pairing.mint().code;
