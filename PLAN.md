# Pinback: Chrome Extension + MCP Server

**Status: agreed 2026-07-06**

Pinback is a visual-feedback overlay for pinning comments to elements on any web page, which a coding agent reads through a local MCP server. It is the standalone, site-agnostic successor to the in-repo a11y-app package ([PR #417](https://github.com/wildlyinaccurate/a11y-app/pull/417)): a Chrome extension carrying the existing Pinback UI, plus a local MCP server as the easiest path for getting comments from the browser into the agent. Architecture inspired by [onUI](https://github.com/onllm-dev/onUI), UI kept from Pinback, and the security concerns raised in [a11y-app PR #411](https://github.com/wildlyinaccurate/a11y-app/pull/411) addressed by design.
...
- **Pairing**: ask-your-agent-for-a-code — the `get_pairing_code` tool mints a short-lived, single-use code the user types into the extension, which exchanges it at `POST /pair` for the long-lived token (server generates `~/.claudback/token`). A code is the only route in: the options page has no token field, so the token is never handled by the user.
...
| Running by default | Off by default at both ends: per-tab opt-in in the extension popup; collector exists only while the agent runs the MCP server. |
| Port config ambiguity | Port 57463 defined once in `packages/shared`; if taken, the server fails fast with a clear error (the extension can only reach the fixed port, so a silent fallback would just misreport as "offline"). |
| `mode` default mismatch | Missing `mode` parses as `"clear"`; store schema validated on every read. |
| Guessing the `/pair` bootstrap code | 8-char code from a 32-symbol alphabet (2^40), 10-minute TTL, single-use, one active code at a time, 5-failed-attempt cap, timing-safe compare, constant ~250 ms failure delay; the endpoint is still origin-gated and loopback-only, and no code exists until one is explicitly minted. |
...
Node 20+, `@modelcontextprotocol/sdk`, stdio. Registered with `claude mcp add pinback -- node <clone>/packages/mcp-server/dist/bin.js` (npm/`npx` later).
...
- **Content script**: the Pinback `widget.ts` overlay (Shadow DOM, pins, composer, list panel, edit/delete); server I/O goes through `chrome.runtime.sendMessage`; `selector.ts` capture unchanged; renders resolved state on pins.
...
3. **Extension** — Sonnet does MV3 boilerplate, `widget.ts` port, worker buffer/sync; Lead reviews manifest permissions and token handling. Then npm publish of `pinback-mcp`.
4. **Post-v1 polish** — Native Messaging transport option, Firefox/Edge builds, Web Store listing, go-public decision, region screenshots, clipboard copy-export fallback. Also finish the npm publish of `pinback-mcp` (*prepped: package is publish-ready, Web Store zip + listing drafted, first-run onboarding built, docs site in `docs/` — see RELEASING.md*): once public, a normal person never has to clone this repo to run the server — `npx pinback-mcp` runs it directly, and a one-line registration (`claude mcp add pinback -- npx -y pinback-mcp`, `codex mcp add pinback -- npx -y pinback-mcp`, or any MCP client's equivalent) has the agent spawn/stop the server itself per session (no daemon needed — the extension buffers offline, and a keep-alive supervisor would hold port 57463 and starve the agent's own instance). Combined with the Web Store listing, this is the cheaper, higher-leverage step before the hosted server below — it fixes the two real friction points (installing the extension, running the server) without new infrastructure, and the whole setup collapses to one prompt a person can hand to their agent: "add the pinback MCP server."
